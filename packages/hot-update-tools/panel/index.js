        ...
        window.plugin = new window.Vue({
            ...
            methods: {
                ...
                onBtnClickPackVersion() {
                    if(this.debugType !== "debug" && this.debugType === "release") {
                        this._addLog("[Error] 测试类型错误");
                        return;
                    }
                ...
                _packageVersion() {
                    ...
                    let zipName = this.debugType+"_" + versionStr + ".zip";
                ...
                onClickGenCfg(event) {
                    ...
                    if (!this.debugType || this.debugType.length <= 0) {
                        this._addLog("[生成] 测试类型未填写");
                        return;
                    }
                    ...
                    this._saveConfig();
                    this._genVersion(this.version,this.debugType, this.serverRootDir, this.resourceRootDir, this.genManifestDir);
                },
                // serverUrl 必须以/结尾
                ...
                _genVersion(version,debugType, serverUrl, buildResourceDir, genManifestDir) {
                ...
                onInputDebugOver(){
                    let debugType = this.debugType;
                    if(debugType !== "debug" && debugType !== "release"){
                        this._addLog("[ERROR] 测试类型只能是debug或者release");
                ...
                onInPutUrlOver(event) {
                    ...
                    this._updateShowUseAddrBtn();
                    let arrayA = this.serverRootDir.split('/');
                    this.debugType = arrayA[arrayA.length-1];
                    this._saveConfig();
                },
                // 获取远程资源服务器的版本号
                ...
                onLogIp() {
                    // getSelectIp();
                    Editor.Ipc.sendToMain('hot-update-tools:test', 'Hello, this is simple panel');
                    // let relativePath = path.relative(__dirname, path.join(Editor.Project.path, 'assets'));
